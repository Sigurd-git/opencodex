...
async function startChild(upstreamUrl: string, dir: string): Promise<ChildHandle> {
  ...
  let gcWaiter: ((receipt: { at: number; durationMs: number }) => void) | null = null;
      ...
      for (const line of lines) {
        try {
          ...
          if (value.type === "gc" && gcWaiter && typeof value.at === "number" && typeof value.durationMs === "number") {
            const w = gcWaiter; gcWaiter = null; w({ at: value.at, durationMs: value.durationMs });
          }
        } catch { /* startup noise */ }
      }
    }
  ...
  try {
    ...
    if (kind === "rss") {
      ...
      return {
        ...
        rssAfterLoad, rssPlus5s, rssPlus60s,
        gcDurationMs: gcReceipt?.durationMs ?? null,
        recoveredByPlus60s: rssAfterLoad - rssPlus60s,
      };
    }
